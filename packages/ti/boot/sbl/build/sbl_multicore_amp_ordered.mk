...
APP_NAME = sbl_multicore_amp_ordered
BUILD_OS_TYPE = baremetal

SRCDIR      += $(PDK_SBL_COMP_PATH)/example/k3MulticoreApp

# Local name of SBL test app
RPRC_PREFIX = sbl_$(BUILD_OS_TYPE)_boot_test_ordered_$(BOARD)

MULTICORE_IMG_PARAMS = $(foreach SOC_CORE_ID, $(sbl_boot_test_ordered_$(SOC)_CORELIST), $(SBL_CORE_ID_$(SOC_CORE_ID)) $(BINDIR)/$(RPRC_PREFIX)_$(SOC_CORE_ID)TestApp_$(BUILD_PROFILE_$(CORE)).rprc)
...
ifneq ($(OS),Windows_NT)
	...
	$(SBL_CERT_GEN) -b $(BINDIR)/$(RPRC_PREFIX)_all_coresTestApp_$(BUILD_PROFILE_$(CORE)).appimage -o $(BINDIR)/$(RPRC_PREFIX)_all_coresTestApp_$(BUILD_PROFILE_$(CORE)).appimage.signed -c R5 -l $(SBL_RUN_ADDRESS) -k $(SBL_CERT_KEY_HS)
