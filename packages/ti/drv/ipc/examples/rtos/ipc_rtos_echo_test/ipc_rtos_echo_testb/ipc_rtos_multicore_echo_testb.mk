...
APP_NAME = ipc_rtos_multicore_echo_testb_$(BUILD_OS_TYPE)

SRCDIR      += $(PDK_IPC_COMP_PATH)/examples/ipc_rtos_echo_test/ipc_rtos_echo_testb

# Local name of IPC test app
RPRC_PREFIX = ipc_rtos_echo_testb

define BIN_PATH_PREFIX_RULE

BIN_PATH_PREFIX_$(1) =  $(pdk_PATH)/ti/binary/$(RPRC_PREFIX)_$(BUILD_OS_TYPE)/bin/$(BOARD)/$(RPRC_PREFIX)_$(BUILD_OS_TYPE)
...
ifneq ($(OS),Windows_NT)
	...
	$(SBL_CERT_GEN) -b $(BINDIR)/$(RPRC_PREFIX)_$(BUILD_OS_TYPE)_all_cores_$(BUILD_PROFILE_$(CORE)).appimage -o $(BINDIR)/$(RPRC_PREFIX)_$(BUILD_OS_TYPE)_all_cores_$(BUILD_PROFILE_$(CORE)).appimage.hs_fs -c R5 -l $(SBL_RUN_ADDRESS) -k $(SBL_CERT_KEY)
